...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const redirectUri = `${process.env.NEXT_PUBLIC_SITE_URL || req.nextUrl.origin}/api/auth/oauth/linkedin/callback`
    ...
    const meRes = await fetch('https://api.linkedin.com/v2/me', {
      headers: { Authorization: `Bearer ${tokenData.access_token}`, 'LinkedIn-Version': '202312' },
    })

    const meData: any = await meRes.json()
    const accountId = meData.id
    const displayName =
      `${meData.localizedFirstName} ${meData.localizedLastName}`.trim() || 'LinkedIn Account'
    ...
    return NextResponse.redirect(new URL('/studio/promote?linkedin=connected', req.url))
  } catch (e: any) {
