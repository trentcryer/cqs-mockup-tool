...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Insert share
    await (admin.from('feed_post_shares') as any).insert({
      user_id: user.id,
      post_id,
    })
    ...
    return NextResponse.json({ success: true, share_count: newCount })
  } catch (e: any) {
    // Duplicate share is OK
    if (e.message.includes('unique')) {
      return NextResponse.json({ success: true, share_count: 0 })
    }
